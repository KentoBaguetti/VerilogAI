...
// Build Verilog-specific context for AI prompts
export const buildVerilogPromptContext = (text: string): string => {
    const context = detectVerilogContext(text);
    const lines = text.split("\n");
    const recentLines = lines.slice(-10).join("\n");

    let contextHint = "";

    if (context === "always_block") {
        contextHint =
            "// Context: Inside an always block. Focus on sequential or combinational logic.\n";
    } else if (context === "module") {
        contextHint =
            "// Context: Inside a Verilog module. Focus on module structure, ports, and internal logic.\n";
    } else if (context === "assign") {
        contextHint =
            "// Context: Continuous assignment. Focus on combinational logic expressions.\n";
    } else {
        contextHint = "// Context: Verilog HDL code.\n";
    }

    return contextHint + recentLines;
};

// Extract module name and ports for better context
export const extractModuleInfo = (
    text: string
): { name: string; ports: string[] } | null => {
    const moduleMatch = text.match(/module\s+(\w+)\s*\(([\s\S]*?)\);/);
    if (!moduleMatch) return null;

    const name = moduleMatch[1];
    const portsSection = moduleMatch[2];
